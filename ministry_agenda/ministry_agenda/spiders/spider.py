...
class MinistrySpider(scrapy.Spider):
    name = 'Ministryspider'
    start_urls = ['http://www.economia.gov.br/agendas/gabinete-do-ministro/ministro-da-economia/paulo-guedes/2020-03-13/']
    ...
    def parse(self, response):
        ...
        self.salva_csv(event_title, event_date, event_location, event_participants, d_participants)
    ...
    def salva_csv(self, et, ed, el, ep, dp):
        df = pd.DataFrame(columns = None)
        for i in range(0, len(et)):
            dicionario = {'MinistryName': 'Ministério da Economia', 'MinisterName': 'Paulo Guedes', 
            'EventDate': ed[i] , 'EventTitle': et[i], 'EventLocation': el[i].replace('\nLocal:', ' '), 'EventParticipants': [dp[i]]}
    ...
    def trata_participantes(self, response, ep):
        ...
        for i in range(0, len(events)):
            if events[i].find("comprimisso-participantes") == -1:
                dicionario[i] = "N/A"
            else:
                dicionario[i] = events[i].find("comprimisso-participantes")
